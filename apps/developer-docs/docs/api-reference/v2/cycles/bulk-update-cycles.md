---
title: Bulk update cycles
description: Bulk update up to 50 cycles in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update cycles, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/cycles/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** cycles in one request, each addressed by `id`. Every row goes through the same path as a single [Update a cycle](/api-reference/v2/cycles/update-cycle).
...
The project the cycles belong to. Accepts the project UUID or its bare identifier, for example `ENG`.
...
The changes to apply. Each entry is the same body as [Update a cycle](/api-reference/v2/cycles/update-cycle) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
| `403`  | `forbidden`              | Your role or token scope can't write cycles in this project.                                       |
...
<CodePanel title="Bulk update cycles" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/cycles/bulk-update/" \
...
```json
{
  "results": [
    ...
    },
    {
      ...
      "type": "not_found",
      "code": "not_found",
      "detail": "No cycle matches the given query."
    }
  ],
  "succeeded": 1,
  "failed": 1
  ...
  "errors": [
    {
      "field": "items.1.name",
      "code": "not_found",
      "message": "No cycle matches the given query."
    }
  ]
}
```
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a cycle](/api-reference/v2/cycles/upsert-cycle) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a cycle body. See [Sparse fields](/api-reference/v2/sparse-fields).
