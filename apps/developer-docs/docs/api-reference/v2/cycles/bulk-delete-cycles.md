---
title: Bulk delete cycles
description: Bulk delete up to 50 cycles in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-delete cycles, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/cycles/bulk-delete/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Delete up to **50** cycles in one request. Each row goes through the same path as a single [Delete a cycle](/api-reference/v2/cycles/delete-cycle), so delete guards still apply per row.
...
The ids of the cycles to delete. Between 1 and **50** per call. A value that isn't a UUID is a `400` for the whole envelope, not a failed row.
...
<CodePanel title="Bulk delete cycles" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/cycles/bulk-delete/" \
