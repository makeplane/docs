...
description: Bulk update up to 50 labels in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update labels, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/labels/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** labels in one request, each addressed by `id`. Every row goes through the same path as a single [Update a label](/api-reference/v2/labels/update-label).
...
The changes to apply. Each entry is the same body as [Update a label](/api-reference/v2/labels/update-label) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
| `403`  | `forbidden`              | Your role or token scope can't write labels in this project.                                       |
...
<CodePanel title="Bulk update labels" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/labels/bulk-update/" \
  ...
  "errors": [
    {
      "field": "items.1.name",
      "code": "not_found",
      "message": "No label matches the given query."
    }
  ]
}
```
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a label](/api-reference/v2/labels/upsert-label) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a label body. See [Sparse fields](/api-reference/v2/sparse-fields).
