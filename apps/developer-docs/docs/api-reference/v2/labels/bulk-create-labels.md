...
description: Bulk create up to 50 labels in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-create labels, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/labels/bulk-create/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Create up to **50** labels in one request. Each row is validated and written exactly as [Create a label](/api-reference/v2/labels/create-label) would, so every business rule and permission check still applies per row.
...
<CodePanel title="Bulk create labels" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/labels/bulk-create/" \
