---
title: List stickies
description: List stickies with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, list stickies, stickies, stickies list
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/stickies/</span>
</div>
...
<ApiParam name="owner_id" type="string (uuid)" :required="false">
...
<CodePanel title="List stickies" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X GET \
  "https://api.plane.so/api/v2/workspaces/my-team/stickies/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/stickies/", {
  method: "GET",
  headers: {
