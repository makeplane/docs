...
description: List teamspaces with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, list teamspaces, teamspaces, teamspaces list
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/teamspaces/</span>
</div>
...
<CodePanel title="List teamspaces" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X GET \
  "https://api.plane.so/api/v2/workspaces/my-team/teamspaces/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/teamspaces/", {
  method: "GET",
  headers: {
