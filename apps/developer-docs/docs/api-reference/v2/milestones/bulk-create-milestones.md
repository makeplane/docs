---
title: Bulk create milestones
description: Bulk create up to 50 milestones in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-create milestones, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/milestones/bulk-create/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Create up to **50** milestones in one request. Each row is validated and written exactly as [Create a milestone](/api-reference/v2/milestones/create-milestone) would, so every business rule and permission check still applies per row.
...
The milestones to create — each entry takes the same body as [Create a milestone](/api-reference/v2/milestones/create-milestone). Between 1 and **50** per call.
...
<CodePanel title="Bulk create milestones" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/milestones/bulk-create/" \
      ...
      "errors": [
        {
          "field": "name",
          "code": "unique",
          "message": "A milestone with this name already exists."
        }
      ]
    }
  ],
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a milestone](/api-reference/v2/milestones/upsert-milestone) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a milestone body. See [Sparse fields](/api-reference/v2/sparse-fields).
