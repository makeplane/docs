---
title: Bulk update milestones
description: Bulk update up to 50 milestones in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update milestones, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/milestones/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** milestones in one request, each addressed by `id`. Every row goes through the same path as a single [Update a milestone](/api-reference/v2/milestones/update-milestone).
...
The changes to apply. Each entry is the same body as [Update a milestone](/api-reference/v2/milestones/update-milestone) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
<CodePanel title="Bulk update milestones" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/milestones/bulk-update/" \
