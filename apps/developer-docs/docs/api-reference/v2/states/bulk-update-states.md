---
title: Bulk update states
description: Bulk update up to 50 states in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update states, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/states/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** states in one request, each addressed by `id`. Every row goes through the same path as a single [Update a state](/api-reference/v2/states/update-state).
...
The project the states belong to. Accepts the project UUID or its bare identifier, for example `ENG`.
...
The changes to apply. Each entry is the same body as [Update a state](/api-reference/v2/states/update-state) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
| `403`  | `forbidden`              | Your role or token scope can't write states in this project.                                       |
...
<CodePanel title="Bulk update states" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/states/bulk-update/" \
  ...
  "errors": [
    {
      "field": "items.1.name",
      "code": "not_found",
      "message": "No state matches the given query."
    }
  ]
}
```
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a state](/api-reference/v2/states/upsert-state) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a state body. See [Sparse fields](/api-reference/v2/sparse-fields).
