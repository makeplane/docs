---
title: Bulk create states
description: Bulk create up to 50 states in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-create states, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/states/bulk-create/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Create up to **50** states in one request. Each row is validated and written exactly as [Create a state](/api-reference/v2/states/create-state) would, so every business rule and permission check still applies per row.
...
The states to create — each entry takes the same body as [Create a state](/api-reference/v2/states/create-state). Between 1 and **50** per call.
...
<CodePanel title="Bulk create states" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/states/bulk-create/" \
