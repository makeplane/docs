---
title: Create a workspace view
description: Create a workspace view with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, create a workspace view, workspace views, workspace views create
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/views/</span>
</div>
...
<CodePanel title="Create a workspace view" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/views/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/views/", {
  method: "POST",
  headers: {
