---
title: Bulk update estimates
description: Bulk update up to 50 estimates in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update estimates, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/estimates/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** estimates in one request, each addressed by `id`. Every row goes through the same path as a single [Update a estimate](/api-reference/v2/estimates/update-estimate).
...
The changes to apply. Each entry is the same body as [Update a estimate](/api-reference/v2/estimates/update-estimate) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
<CodePanel title="Bulk update estimates" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/estimates/bulk-update/" \
  ...
  "errors": [
    {
      "field": "items.1.name",
      "code": "not_found",
      "message": "No estimate matches the given query."
    }
  ]
}
```
