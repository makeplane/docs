---
title: Bulk create estimates
description: Bulk create up to 50 estimates in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-create estimates, bulk write, all_or_none, partial success, per-row results
---

# Bulk create estimates

<div class="api-endpoint-badge">
  <span class="method post">POST</span>
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/estimates/bulk-create/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Create up to **50** estimates in one request. Each row is validated and written exactly as [Create a estimate](/api-reference/v2/estimates/create-estimate) would, so every business rule and permission check still applies per row.
...
The estimates to create — each entry takes the same body as [Create a estimate](/api-reference/v2/estimates/create-estimate). Between 1 and **50** per call.
...
| `403`  | `forbidden`              | Your role or token scope can't write estimates in this project.                                    |
...
<CodePanel title="Bulk create estimates" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/estimates/bulk-create/" \
      ...
      "errors": [
        {
          "field": "name",
          "code": "unique",
          "message": "A estimate with this name already exists."
        }
      ]
    }
  ],
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a estimate](/api-reference/v2/estimates/upsert-estimate) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a estimate body. See [Sparse fields](/api-reference/v2/sparse-fields).
