---
title: Bulk update estimate points
description: Bulk update up to 50 estimate points in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update estimate points, bulk write, all_or_none, partial success, per-row results
---

# Bulk update estimate points

<div class="api-endpoint-badge">
  <span class="method post">POST</span>
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/estimates/{estimate_id}/points/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** estimate points in one request, each addressed by `id`. Every row goes through the same path as a single [Update a estimate point](/api-reference/v2/estimate-points/update-estimate-point).
...
The changes to apply. Each entry is the same body as [Update a estimate point](/api-reference/v2/estimate-points/update-estimate-point) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
<CodePanel title="Bulk update estimate points" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/estimates/9c1f0b3d-6d2e-4c9a-8b41-2f7e5a0d6c88/points/bulk-update/" \
