---
title: Bulk create estimate points
description: Bulk create up to 50 estimate points in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-create estimate points, bulk write, all_or_none, partial success, per-row results
---

# Bulk create estimate points

<div class="api-endpoint-badge">
  <span class="method post">POST</span>
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/estimates/{estimate_id}/points/bulk-create/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Create up to **50** estimate points in one request. Each row is validated and written exactly as [Create a estimate point](/api-reference/v2/estimate-points/create-estimate-point) would, so every business rule and permission check still applies per row.
...
The estimate points to create — each entry takes the same body as [Create a estimate point](/api-reference/v2/estimate-points/create-estimate-point). Between 1 and **50** per call.
...
<CodePanel title="Bulk create estimate points" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/estimates/9c1f0b3d-6d2e-4c9a-8b41-2f7e5a0d6c88/points/bulk-create/" \
      ...
      "errors": [
        {
          "field": "name",
          "code": "unique",
          "message": "A estimate point with this name already exists."
        }
      ]
    }
  ],
