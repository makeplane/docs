---
title: Create a release label
description: Create a release label with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, create a release label, release labels, release labels create
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/releases/labels/</span>
</div>
...
<CodePanel title="Create a release label" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/releases/labels/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/releases/labels/", {
  method: "POST",
  headers: {
