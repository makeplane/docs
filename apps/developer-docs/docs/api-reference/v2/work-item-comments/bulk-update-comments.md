...
description: Bulk update up to 50 comments in one call with the Plane v2 REST API. Per-row results, atomic all_or_none mode, OAuth scopes, error codes, and code examples.
keywords: plane api v2, bulk-update comments, bulk write, all_or_none, partial success, per-row results
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/work-items/{work_item_id}/comments/bulk-update/</span>
</div>

<div class="api-two-column">
<div class="api-left">

Update up to **50** comments in one request, each addressed by `id`. Every row goes through the same path as a single [Update a comment](/api-reference/v2/work-item-comments/update-comment).
...
The changes to apply. Each entry is the same body as [Update a comment](/api-reference/v2/work-item-comments/update-comment) **plus a required `id`** naming the row to update. Between 1 and **50** per call.
...
| `403`  | `forbidden`              | Your role or token scope can't write comments in this project.                                     |
...
<CodePanel title="Bulk update comments" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/work-items/b7e42a19-3c5d-4f80-9a26-8d1c0f4e7b53/comments/bulk-update/" \
  ...
  "items": [
    {
      ...
      "comment_html": "<p>Reproduced on staging and prod.</p>"
    },
    {
  ...
  "errors": [
    {
      "field": "items.1.name",
      "code": "not_found",
      "message": "No comment matches the given query."
    }
  ]
}
```
...
- **Reconciling on external ids is not bulk's job.** Use [Upsert a comment](/api-reference/v2/work-item-comments/upsert-comment) per row, or list and diff.
- `?fields=` does not apply here — the response is a per-row result envelope, not a comment body. See [Sparse fields](/api-reference/v2/sparse-fields).
