...
description: Create a workspace page with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, create a workspace page, workspace pages, workspace pages create
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/pages/</span>
</div>
...
<CodePanel title="Create a workspace page" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/pages/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/pages/", {
  method: "POST",
  headers: {
