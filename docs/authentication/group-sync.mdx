...
:::info
Group syncing is currently available on the self-hosted Commercial Edition only and supports OIDC. We are also working on adding support for SAML and LDAP.
:::
...
2. Toggle on **Enable group syncing**.
  ...
  ![Group syncing](https://media.docs.plane.so/sso/group-syncing.webp#hero)
...
![Add new group sync](https://media.docs.plane.so/sso/add-group-sync.webp#hero)
