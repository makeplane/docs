...
With the GitHub integration set up, you can start syncing issues between Plane and GitHub. The synchronization is bi-directional and label-based, so issues are updated automatically on both platforms when certain labels are applied.
...
- **Closing references**  
These keywords closes linked issues when the PR is merged or closed.
**Keywords**  
