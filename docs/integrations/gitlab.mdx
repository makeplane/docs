...
Integrating GitLab with Plane allows you to sync your development workflow seamlessly by linking GitLab merge requests with Plane issues. This connection enables automated updates and enhances collaboration between developers and project managers. 
...
If you're running a self-hosted instance of Plane, you'll need to set up a few extra configurations to get Slack integration working. Check out the [setup guide](https://developers.plane.so/self-hosting/govern/integrations/gitlab) first before diving into the steps on this page.
:::
...
After linking a GitLab project, the next step is to associate it with a Plane project. This connection ensures that Plane issues can be updated automatically based on GitLab merge request activities.
...
2. In the **Pull Request Automation** section, set up rules for how Plane should update issue states based on merge request lifecycle events.
