...
For more advanced admins that want greater control over their instance, the list of additional commands available on Prime CLI follow.
    ...
    - `healthcheck`
        ...
        Reports on the the status of all running service

    - `monitor`

        Displays the logs and the statuses of all running services

    - `repair`

        Fixes your Plane install for common errors.

    - `update-cli`

        Upgrades the Prime CLI to its latest version
