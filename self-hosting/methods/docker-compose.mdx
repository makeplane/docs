...
Before proceeding, we suggest you to review `plane.env` file and set the values. Below are the most import keys you must refer to. _You can use any text editor to edit this file_.
...
In case you want to make changes to `plane.env` variables, without stopping the server or you noticed some abnormalities in services, you can restart the services with RESTART option.
...
By choosing this, it will stop the services and then will download the latest `docker-compose.yaml` and `variables-upgrade.env`. Here system will not replace `plane.env` with the new one.
...
> _**"It is very important for you to compare the 2 files `variables-upgrade.env` and `plane.env`. Copy the newly added variable from downloaded file to `plane.env` and set the expected values."**_
