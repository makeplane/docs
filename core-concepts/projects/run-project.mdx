...
- **Type**: Type of the estimate system to be used in the project.
    - **Category**: Categorical estimates are a set of strings, which can be used in your project to estimate work. Typical examples of these are T-shirt sizing estimates or easy to hard estimate systems.
    - **Points**: Point estimates are a set of numbers to estimate issues. With numbers, you can create a linear sequence, a fibonacci sequence or any other sequence of your choice to estimate work.
- **Estimate**: Each selectable value for a member of the project to assign to their issues. You can add, re-order or remove any estimate.

#### Archived estimates
In older versions we supported adding different estimate groups, to help you switch between using different estimate values for your project easily. We have now changed this behavior to only have a single estimate group for your project. In-active estimate groups in the older version are now archived.
